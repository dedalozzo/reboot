...
<ul class="list tabs {{ tabName === actionName ? 'half-gutter' : 'gutter'}}">
  ...
  {% if etag is defined %}
    {% set tagRoute = etag.name|url_encode~'/' %}
    <li><span>{% if user.isMember() %}<a id="{{ etag.id }}" class="btn-star{% if user.tags.exists(etag.unversionId) %} active{% endif %}" title="add to favorites"><i class="icon-star icon-large"></i></a>&nbsp;{% endif %}<a class="tag" href="{{ '//'~domainName~'/'~tagRoute }}">{{ etag.name }}</a></span></li>
  {% else %}
    {% set tagRoute = '' %}
  {% endif %}
