...
<ul class="list tabs">
  {% if resource is defined %}
    <li><span>{% if etag is defined %}<a id="{{ etag.id }}" class="btn-star{% if etag.isStarred() %} active{% endif %}" title="add to favorites"><i class="icon-star icon-large"></i></a>{% endif %}&nbsp;<b>{{ resource }}</b></span></li>
