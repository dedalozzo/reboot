          ...
          <table id="emails" class="std gutter">
            ...
            {% for email, verified in emails %}
              <tr>
                ...
                {% if not verified %}
                <td><button name="resendVerificationEmail" type="submit" value="{{ email }}" class="btn btl-link">Resend verification e-mail</button>.</td>
                {% elseif not primary %}
                <td><button name="setAsPrimaryEmail" type="submit" value="{{ email }}" class="btn btl-link">Set as primary e-mail</button>.</td>
                {% else %}
                <td></td>
                {% endif %}
                ...
                <td>{{ user.canRemoveEmail(email) ? '<button name="removeEmail" type="submit" value="'~email~'" class="btn btn-icon red" title="remove e-mail"><i class="icon-trash icon-large"></i></button>' : '' }}</td>
              </tr>
            {% endfor %}
            </tbody>
          </table>
